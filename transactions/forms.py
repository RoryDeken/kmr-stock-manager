...
class SupplierForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['taxid'].widget.attrs.update({'class': 'textinput form-control', 'maxlength': '15', 'pattern' : '[0-9]{15}', 'title' : 'Numbers only' })
    class Meta:
        model = Supplier
        fields = ['name', 'phone', 'address', 'email', 'taxid']
        widgets = {
...
class SaleForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['email'].widget.attrs.update({'class': 'textinput form-control'})
        self.fields['taxid'].widget.attrs.update({'class': 'textinput form-control'})
    class Meta:
        model = SaleBill
        fields = ['name', 'phone', 'address', 'email', 'taxid']
        widgets = {
